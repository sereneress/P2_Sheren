            ...
            <div class="card-body d-flex flex-column">
                ...
                <form method="POST" action="{{ route('Pemeriksaan.store') }}"
                    ...
                    <div class="mb-2">
                        ...
                        <input type="text" name="suhu_badan" class="form-control" value="{{ $last->suhu_badan ?? '' }}">
                    </div>

                    <div class="mb-2">
                        <label>Tensi</label>
                        <input type="text" name="tensi" class="form-control" value="{{ $last->tensi ?? '' }}">
                    </div>

                    <div class="mb-2">
                        <label>Keluhan</label>
                        <textarea name="keluhan" class="form-control">{{ $last->keluhan ?? '' }}</textarea>
                    </div>

                    <div class="mb-2">
                        <label>Catatan Obat Dokter</label>
                        <textarea name="catatan_obat" class="form-control">{{ $last->catatan_obat ?? '' }}</textarea>
                    </div>
